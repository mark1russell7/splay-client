 ...
 * ```
 */
export function createClientRegistry(call, options = {}) {
    ...
    function createRenderer(type) {
        ...
        return async (ctx) => {
            ...
            // Call the procedure
            const result = await call(procedurePath, input);
            return result;
        };
    }
    ...
    return {
         ...
         */
        register(type, _renderer) {
            throw new Error(`Cannot register renderers directly on ClientRegistry. ` +
 ...
 */
export function createCheckedRegistry(call, hasProcedure, options = {}) {
 ...
 */
export function createRenderContext(registry, data, size, path = "root", depth = 0) {
    return {
        data,
        size,
        path,
        depth,
        render: async (childData, childSize, childPath) => {
            ...
            const childCtx = createRenderContext(registry, childData, childSize, childPath, depth + 1);
